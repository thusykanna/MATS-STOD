...
def test_b0_sends_no_context(settings, pair):
...
def test_retry_once_then_succeed(settings):
    ...
    record = translator.translate_segment(seg, ContextBlock(text=""), "B0_full_document")
...
def test_persistent_parse_failure_falls_back_and_is_flagged(settings):
    ...
    record = Translator(settings, llm).translate_segment(seg, ContextBlock(text=""), "B0_full_document")
...
def test_translation_record_captures_provenance(settings, pair):
    ...
    assert r.context_strategy == "B0_full_document"
